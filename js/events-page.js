...
const eventsData = {
  'event-1': {
    ...
    status: 'open' // 'open', 'ongoing', or 'concluded'
  },
  'event-2': {
...
// ===== SETUP "SEE DETAILS" BUTTONS ON HOMEPAGE/PROFILE PAGE =====
...
function updateEventDetails(eventId) {
  ...
  if (statusDot && statusText && event.status) {
    // Remove all status classes
    statusDot.classList.remove('open', 'ongoing', 'concluded');
    
    // Add the appropriate status class
    statusDot.classList.add(event.status);
    
    // Update status text (capitalize first letter)
    statusText.textContent = event.status.charAt(0).toUpperCase() + event.status.slice(1);
  }
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Check if we're on the events details page
